...
- **MCP Chat Interface**: AI-powered chat assistant that can answer questions about your blog content
...
The Memory and Computation Protocol (MCP) server enables AI-powered chat functionality for your blog, allowing visitors to ask questions about your content and receive relevant answers.

### Prerequisites

1. **OpenAI API Key**: You'll need an API key from OpenAI to use the embeddings and chat completion features.
...
2. **Generate embeddings for your content**:
   ...
   Run the following command to generate vector embeddings for your blog posts:
   ```bash
   npm run generate-embeddings
   ```
   
   This will create a vector store in the `data/vector-store` directory.

3. **Build and deploy your site**:
   
   The MCP server will automatically be included in your build when you run:
   ```bash
   npm run build
   ```
...
Once configured and deployed, visitors can access the chat interface at `/chat`. The interface allows users to:

- Ask questions about your blog content
- Get contextually relevant responses based on your writing
- See source references that link back to your original posts

### Customization

You can customize the AI assistant's persona by editing the `SYSTEM_MESSAGE` in `src/pages/api/chat.js`.