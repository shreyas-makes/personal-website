...
import { FileChange } from './documentationSession';

export class FileChangeTracker {
    private fileSystemWatcher: vscode.FileSystemWatcher | undefined;
    private fileContents: Map<string, string> = new Map();
    private onChangeCallback: ((change: FileChange) => void) | undefined;
    private config = vscode.workspace.getConfiguration('ai-docs');

    startTracking(onChangeCallback: (change: FileChange) => void): void {
    ...
    private async cacheDirectoryContents(uri: vscode.Uri): Promise<void> {
        try {
    ...
    private async handleFileCreate(uri: vscode.Uri): Promise<void> {
        if (!this.shouldTrackFile(uri.fsPath) || !this.onChangeCallback) return;
    ...
    private generateDiff(oldContent: string, newContent: string, filePath: string): string {
        const maxLines = this.config.get<number>('maxDiffLines', 10);
        
        const patch = createTwoFilesPatch(
            `a/${this.getRelativePath(filePath)}`,
            `b/${this.getRelativePath(filePath)}`,
            oldContent,
            newContent,
            '',
            '',
    ...
    private generateCreationDiff(content: string): string {
        const maxLines = this.config.get<number>('maxDiffLines', 10);
    ...
    private generateDeletionDiff(content: string): string {
        const maxLines = this.config.get<number>('maxDiffLines', 10);
        const lines = content.split('\n');
